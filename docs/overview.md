...
7. **编译期优先，运行期兜底**：静态类型系统保证大部分区域隔离；涉及字段/槽打开、并发所有者释放或 FFI/unsafe 边界时允许运行期检查给出确定性错误。
8. **先收敛安全核心**：Core Gura v0 暂不支持 active region 跨 await、mutable global、safe 逐对象 free、read cown、隐式冻结和会逃逸的 region borrow closure。
...
`acquire` 块结束时，cown 内部值必须再次闭合；否则编译器或运行时报告“区域逃逸”。多 cown 获取按运行时全序排他获取；panic/unwind 路径也必须释放 cown，不能把半开放区域暴露给其他线程。
