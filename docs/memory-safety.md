...
enter users as u {
    explore log as l {
        let count: i64 = l.count()
        u.cached_log_count = count
    }
}
```

进入 `users` 后，`R_users` 是 active。再 `explore log` 时，`R_log` 被打开为只读视图，而 `R_users` 在概念上仍是外层上下文：

```text
inside explore log:
  R_log: paused/read-only view, l: pau Log
  R_users: active work context for body-local mutation/allocation
...
禁止：
  l.append(...)
  new mut Entry(...) inside R_log
```

更一般地，当一个已打开 region 被更内层 `enter` 挂起时，它就是 paused：可以被更内层临时读取，但不能被写入。这样保证外层 region 的桥路径和内部拓扑在内层 active 期间保持稳定。
